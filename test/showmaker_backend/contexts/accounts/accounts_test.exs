defmodule ShowmakerBackend.Contexts.Accounts.AccountsTest do
  @moduledoc false
  ...
  describe "get_account_by_email/1" do
    test "does not return the account if the email does not exist" do
      refute Accounts.get_account_by_email("unknown@showmaker.com")
    end
  ...
  describe "register_account/1" do
    ...
    test "registers accounts with a hashed password" do
      ...
      assert {:ok, account} = Accounts.register_account(valid_account_attributes(email: email))
  ...
  #     assert account_token.context == "change:current@showmaker.com"
  #   end
  # end
  ...
  #     assert Accounts.update_account_email(%{account | email: "current@showmaker.com"}, token) ==
  #              :error
  ...
  describe "create_user_api_token/1 and fetch_user_by_api_token/1" do
    test "does not return account from invalid token" do
  ...
  describe "delete_access_token_by_account/1" do
    setup do
      %{account: account_fixture()}
    end

    test "return ok even when token is invalid", %{account: account} do
      query =
        account
        |> AccountToken.by_account_and_contexts_query(:all)

      assert Repo.all(query) == []
      assert Accounts.delete_access_token_by_account(account) == :ok
      assert Repo.all(query) == []
    end

    test "creates and fetches by token", %{account: account} do
      query =
        account
        |> AccountToken.by_account_and_contexts_query(:all)
      ...
      Accounts.create_account_api_token(account)

      assert [%{account_id: ^account_id}] = Repo.all(query)
      assert Accounts.delete_access_token_by_account(account) == :ok
      assert Repo.all(query) == []
    end
  end
